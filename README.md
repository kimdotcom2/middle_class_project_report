...
초기에 DB 모델링에 미흡했기 때문에 JPA 연관 관계를 사용하던 상황에서 의도치 않은 문제가 발생하였다.
스프링 배치로 생성된 인기 도서 엔티티가 도서 엔티티에 의존하면서, 도서 정보를 soft delete 하기 위해서는 의존 관계까지 삭제해줄 필요가 있었다.

자식 엔티티와 연관된 repository를 BookService에 주입하여 BookRating 엔티티와 도서 엔티티를 의존하는 리뷰 엔티티 및 ReviewRating를 먼저 삭제하려고 시도하였으나,
BookRating을 삭제하려고 했을 떄 의도한 delete 쿼리가 아닌 도서 엔티티에 대한 update 쿼리가 생성되며 제약 조건 예외가 발생하였다.

## 해결 과정

우선 왜 ReviewRating을 삭제하려고 하면 update 쿼리가 나가게 되는지 원인을 파악하려고 시도했다.

그 이유는 삭제를 deleteByBook_Id 메소드로 처리하려고 했기 때문이었다.

부모 엔티티의 id를 통해 삭제를 시도하면, JPA는 먼저 부모 엔티티와 자식 엔티티의 연관 관계를 끊기 위해 참조 칼럼의 값을 null로 바꾸려고 한다.

하지만 DB에서는 update 제약 조건이 걸려있기에 참조 에러가 발생했던 것이다.

따라서 update 쿼리 없이 바로 delete 쿼리를 실행하도록 JPQL로 명시적으로 삭제하도록 하였다.
